...
#if defined(UNIX) || defined(_UNIX) || defined(__UNIX__) || defined(__unix) || \
    defined(__unix__) || defined(____SVR4____) || defined(__LINUX__) ||        \
    defined(__sgi) || defined(__hpux) || defined(sun) || defined(__SUN__) ||   \
    defined(_AIX) || defined(__EMX__) || defined(__VMS) || defined(__BEOS__)
#ifndef UNIX
#define UNIX
#endif
#ifndef _UNIX
...
/*-------------------------------------------------------------------------*/
/*64-bit int                                                               */
#if defined(__MINGW32__) || defined(__CYGWIN32__) || defined(__UNIX__) || \
    defined(__MACOSX__)
#undef MAXTYPE_INT
...
#define MEDIAINFO_ASSIGN(_Name, _Name2)                       \
  if (!g_module_symbol(MediaInfo_Module,                      \
                       "MediaInfo" MEDIAINFO_Ansi "_" _Name2, \
                       (gpointer*)&MediaInfo_##_Name))        \
    Errors++;
#define MEDIAINFOLIST_ASSIGN(_Name, _Name2)                       \
  if (!g_module_symbol(MediaInfo_Module,                          \
                       "MediaInfoList" MEDIAINFO_Ansi "_" _Name2, \
                       (gpointer*)&MediaInfoList_##_Name))        \
    Errors++;
#elif defined(_WIN32) || defined(WIN32)
#define MEDIAINFO_ASSIGN(_Name, _Name2)                         \
  MediaInfo_##_Name = (MEDIAINFO_##_Name)GetProcAddress(        \
      MediaInfo_Module, "MediaInfo" MEDIAINFO_Ansi "_" _Name2); \
  if (MediaInfo_##_Name == NULL) Errors++;
#define MEDIAINFOLIST_ASSIGN(_Name, _Name2)                         \
  MediaInfoList_##_Name = (MEDIAINFOLIST_##_Name)GetProcAddress(    \
      MediaInfo_Module, "MediaInfoList" MEDIAINFO_Ansi "_" _Name2); \
  if (MediaInfoList_##_Name == NULL) Errors++;
#else
#define MEDIAINFO_ASSIGN(_Name, _Name2)                         \
  MediaInfo_##_Name = (MEDIAINFO_##_Name)dlsym(                 \
      MediaInfo_Module, "MediaInfo" MEDIAINFO_Ansi "_" _Name2); \
  if (MediaInfo_##_Name == NULL) Errors++;
#define MEDIAINFOLIST_ASSIGN(_Name, _Name2)                         \
  MediaInfoList_##_Name = (MEDIAINFOLIST_##_Name)dlsym(             \
      MediaInfo_Module, "MediaInfoList" MEDIAINFO_Ansi "_" _Name2); \
  if (MediaInfoList_##_Name == NULL) Errors++;
#endif
...
typedef size_t(__stdcall* MEDIAINFOLIST_Open)(void*, const MediaInfo_Char*,
                                              const MediaInfo_fileoptions_C);
static MEDIAINFOLIST_Open MediaInfoList_Open;
typedef size_t(__stdcall* MEDIAINFO_Open_Buffer_Init)(
    void*, MediaInfo_int64u File_Size, MediaInfo_int64u File_Offset);
static MEDIAINFO_Open_Buffer_Init MediaInfo_Open_Buffer_Init;
typedef size_t(__stdcall* MEDIAINFO_Open_Buffer_Continue)(
    void*, MediaInfo_int8u* Buffer, size_t Buffer_Size);
static MEDIAINFO_Open_Buffer_Continue MediaInfo_Open_Buffer_Continue;
typedef MediaInfo_int64u(__stdcall* MEDIAINFO_Open_Buffer_Continue_GoTo_Get)(
    void*);
static MEDIAINFO_Open_Buffer_Continue_GoTo_Get
...
typedef const MediaInfo_Char*(__stdcall* MEDIAINFO_Inform)(void*,
                                                           size_t Reserved);
static MEDIAINFO_Inform MediaInfo_Inform;
typedef const MediaInfo_Char*(__stdcall* MEDIAINFOLIST_Inform)(void*, size_t,
                                                               size_t Reserved);
static MEDIAINFOLIST_Inform MediaInfoList_Inform;
typedef const MediaInfo_Char*(__stdcall* MEDIAINFO_GetI)(
    void*, MediaInfo_stream_C StreamKind, size_t StreamNumber, size_t Parameter,
    MediaInfo_info_C KindOfInfo);
static MEDIAINFO_GetI MediaInfo_GetI;
typedef const MediaInfo_Char*(__stdcall* MEDIAINFOLIST_GetI)(
...
typedef const MediaInfo_Char*(__stdcall* MEDIAINFO_Get)(
    void*, MediaInfo_stream_C StreamKind, size_t StreamNumber,
    const MediaInfo_Char* Parameter, MediaInfo_info_C KindOfInfo,
    MediaInfo_info_C KindOfSearch);
static MEDIAINFO_Get MediaInfo_Get;
typedef const MediaInfo_Char*(__stdcall* MEDIAINFOLIST_Get)(
    void*, size_t, MediaInfo_stream_C StreamKind, size_t StreamNumber,
    const MediaInfo_Char* Parameter, MediaInfo_info_C KindOfInfo,
    MediaInfo_info_C KindOfSearch);
static MEDIAINFOLIST_Get MediaInfoList_Get;
typedef size_t(__stdcall* MEDIAINFO_Output_Buffer_Get)(
    void*, const MediaInfo_Char* Parameter);
...
typedef const MediaInfo_Char*(__stdcall* MEDIAINFO_Option)(
    void*, const MediaInfo_Char* Parameter, const MediaInfo_Char* Value);
static MEDIAINFO_Option MediaInfo_Option;
typedef const MediaInfo_Char*(__stdcall* MEDIAINFOLIST_Option)(
    void*, const MediaInfo_Char* Parameter, const MediaInfo_Char* Value);
...
typedef size_t(__stdcall* MEDIAINFO_Count_Get)(void*,
                                               MediaInfo_stream_C StreamKind,
                                               size_t StreamNumber);
static MEDIAINFO_Count_Get MediaInfo_Count_Get;
typedef size_t(__stdcall* MEDIAINFOLIST_Count_Get)(
    void*, size_t, MediaInfo_stream_C StreamKind, size_t StreamNumber);
...
/* Load library */
  ...
  if (!MediaInfo_Module) {
    ...
    CFURLRef app_path_url =
        CFURLCreateCopyDeletingLastPathComponent(NULL, app_url);

    CFStringRef app_path =
        CFURLCopyFileSystemPath(app_path_url, kCFURLPOSIXPathStyle);

    CFMutableStringRef mut_app_path =
        CFStringCreateMutableCopy(NULL, NULL, app_path);
...
#endif
  if (!MediaInfo_Module) return (size_t)-1;
  ...
  MEDIAINFO_ASSIGN(Open_Buffer_Continue_GoTo_Get,
...
enum infooptions_t {
  ...
  InfoOption_Reserved,         ///<
  InfoOption_ShowInSupported,  ///< Internal use only (info : Must be showed in
                               ///Info_Capacities() )
  InfoOption_TypeOfValue,      ///< Value return by a standard Get() can be : T
                               ///(Text), I (Integer, warning up to 64 bits), F
                           ///(Float), D (Date), B (Binary datas coded Base64)
...
class MediaInfo {
 public:
  MediaInfo() {
    if (!MediaInfo_Module) MediaInfoDLL_Load();
    if (!MediaInfo_Module) {
      Handle = NULL;
      return;
    };
  ...
  size_t Open_Buffer_Init(MediaInfo_int64u File_Size = (MediaInfo_int64u)-1,
                          MediaInfo_int64u File_Offset = 0) {
    MEDIAINFO_TEST_INT;
  ...
  String Get(stream_t StreamKind, size_t StreamNumber, size_t Parameter,
             info_t InfoKind = Info_Text) {
    MEDIAINFO_TEST_STRING;
    return MediaInfo_GetI(Handle, (MediaInfo_stream_C)StreamKind, StreamNumber,
                          Parameter, (MediaInfo_info_C)InfoKind);
  };
  String Get(stream_t StreamKind, size_t StreamNumber, const String& Parameter,
    ...
    return MediaInfo_Get(Handle, (MediaInfo_stream_C)StreamKind, StreamNumber,
                         Parameter.c_str(), (MediaInfo_info_C)InfoKind,
                         (MediaInfo_info_C)SearchKind);
  };
  // size_t Set (const String &ToSet, stream_t StreamKind, size_t StreamNumber,
  // size_t Parameter, const String &OldValue=__T(""))  {MEDIAINFO_TEST_INT;
  // return MediaInfo_SetI (Handle, ToSet.c_str(),
  // (MediaInfo_stream_C)StreamKind, StreamNumber, Parameter,
  // OldValue.c_str());};
  // size_t Set (const String &ToSet, stream_t StreamKind, size_t StreamNumber,
  // const String &Parameter, const String &OldValue=__T(""))
  // {MEDIAINFO_TEST_INT; return MediaInfo_Set (Handle, ToSet.c_str(),
  // (MediaInfo_stream_C)StreamKind, StreamNumber, Parameter.c_str(),
  // OldValue.c_str());};
  ...
  static String Option_Static(const String& Option,
                              const String& Value = __T("")) {
    if (!MediaInfo_Module) MediaInfoDLL_Load();
  ...
  size_t Count_Get(stream_t StreamKind, size_t StreamNumber = (size_t)-1) {
    MEDIAINFO_TEST_INT;
    return MediaInfo_Count_Get(Handle, (MediaInfo_stream_C)StreamKind,
                               StreamNumber);
  };
...
class MediaInfoList {
 public:
  ...
  size_t Open(const String& File,
    ...
    MEDIAINFO_TEST_INT;
    return MediaInfoList_Open(Handle, File.c_str(),
                              (MediaInfo_fileoptions_C)Options);
  };
  // size_t Save (size_t FilePos) {MEDIAINFO_TEST_INT; return
  // MediaInfoList_Save(Handle, FilePos);};
  ...
  String Get(size_t FilePos, stream_t StreamKind, size_t StreamNumber,
    ...
    MEDIAINFO_TEST_STRING;
    return MediaInfoList_GetI(Handle, FilePos, (MediaInfo_stream_C)StreamKind,
                              StreamNumber, Parameter,
  ...
  String Get(size_t FilePos, stream_t StreamKind, size_t StreamNumber,
    ...
    MEDIAINFO_TEST_STRING;
    return MediaInfoList_Get(Handle, FilePos, (MediaInfo_stream_C)StreamKind,
                             StreamNumber, Parameter.c_str(),
                             (MediaInfo_info_C)InfoKind,
                             (MediaInfo_info_C)SearchKind);
  };
  // size_t Set (const String &ToSet, size_t FilePos, stream_t StreamKind,
  // size_t StreamNumber, size_t Parameter, const String &OldValue=__T(""))
  // {MEDIAINFO_TEST_INT; return MediaInfoList_SetI (Handle, ToSet.c_str(),
  // FilePos, (MediaInfo_stream_C)StreamKind, StreamNumber, Parameter,
  // OldValue.c_str());};
  // size_t Set (const String &ToSet, size_t FilePos, stream_t StreamKind,
  // size_t StreamNumber, const String &Parameter, const String
  // &OldValue=__T(""))  {MEDIAINFO_TEST_INT; return MediaInfoList_Set (Handle,
  // ToSet.c_str(), FilePos, (MediaInfo_stream_C)StreamKind, StreamNumber,
  ...
  size_t Count_Get(size_t FilePos, stream_t StreamKind,
                   size_t StreamNumber = (size_t)-1) {
    MEDIAINFO_TEST_INT;
    return MediaInfoList_Count_Get(
        Handle, FilePos, (MediaInfo_stream_C)StreamKind, StreamNumber);
  };
  size_t Count_Get() {
    MEDIAINFO_TEST_INT;
