...
std::string Base64Encode(uint8_t const* buf, unsigned int bufLen) {
  ...
  while (bufLen--) {
    ...
    if (i == 3) {
      ...
      for (i = 0; (i < 4); i++) ret += g_base64_chars[char_array_4[i]];
      i = 0;
    }
  }
  ...
  if (i) {
    ...
    for (j = 0; (j < i + 1); j++) ret += g_base64_chars[char_array_4[j]];
...
std::vector<uint8_t> Base64Decode(std::string const& encoded_string) {
  ...
  while (in_len-- && (encoded_string[in_] != '=') &&
    ...
    if (i == 4) {
      for (i = 0; i < 4; i++)
        char_array_4[i] = (uint8_t)g_base64_chars.find(char_array_4[i]);
  ...
  if (i) {
    ...
    for (j = 0; j < 4; j++)
      char_array_4[j] = (uint8_t)g_base64_chars.find(char_array_4[j]);
