...
///
class ArtistsObserver implements ObserverInterface {
  // Filter
  MediaFilter _filter = MediaFilter.forArtists();
  ...
  // Stream controller.
  StreamController<List<ArtistModel>>? _controller;
  ...
  Future<void> startObserver([Map<String, dynamic>? args]) async {
    ...
    _controller ??= StreamController<List<ArtistModel>>.broadcast(
      onListen: onChange,
    ...
    // already running.
    _controller!.add(
      await _artistsQuery.queryArtists(
