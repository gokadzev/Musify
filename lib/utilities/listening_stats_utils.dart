...
Duration qualifiedPlaybackThreshold(Duration? duration) {
  ...
  if (duration < shortSongBoundary) {
    ...
    const minThreshold = Duration(milliseconds: 500);
    return halfDuration > minThreshold ? halfDuration : minThreshold;
  }
...
Map<String, dynamic> _storeDeltaMonth(
  ...
  if (writesToCurrentMonth) {
    ...
    final writtenMonth = _trimmedMonth(
      trimmedMonth,
      songLimit: wrappedMonthlyHistorySongsLimit,
    );
    history[listenedMonthKey] = writtenMonth;
    // Re-pin the just-written month: retention is evaluated as of listenedAt,
    // under which this month isn't "before" now and would otherwise be pruned.
    final retained = _retainedHistory(history, listenedAt)
      ..[listenedMonthKey] = writtenMonth;
    stats['history'] = retained;
  }
...
void _upsertSongStats(
  ...
  entry
    ...
    // Both spellings are intentional and load-bearing: the app stores the
    // offline artwork path as 'artworkPath' in most places but as 'artWorkPath'
    // in MediaItem extras (see mediaitem.dart / song_artwork.dart). Keep both so
    // a recap song retains its offline cover whichever spelling fed it in.
...
// A history month is kept on disk (and shown in the UI) only while it stays
// relevant: earlier months of the current year, plus — during January — the
// whole previous year so its annual recap is still available. Anything older is
// dropped on the next normalize (e.g. in February the prior year is pruned),
// which bounds stored history and the backup to ~13 months. Retention and
// visibility deliberately share this one rule: a pruned month can't be shown.
bool _shouldRetainHistoryMonth(String monthKey, DateTime now) {
  ...
  return now.month == DateTime.january && parsed.year == now.year - 1;
}
